...
-- Retell agent mappings (cache CaseCraft agent -> Retell agent)
CREATE TABLE IF NOT EXISTS retell_agent_mappings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  casecraft_agent_id UUID NOT NULL REFERENCES agents(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_retell_agent_mappings_agent_id ON retell_agent_mappings(casecraft_agent_id);
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'retell_agent_mappings'
    ...
    CREATE POLICY "Users can view agent mappings" ON retell_agent_mappings FOR ALL USING (
      EXISTS (
        ...
        WHERE a.id = retell_agent_mappings.casecraft_agent_id
...
COMMENT ON TABLE retell_agent_mappings IS 'Maps CaseCraft agents to Retell voice agents';
