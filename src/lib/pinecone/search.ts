...
import { getCurrentPlaintiff, getCurrentDefendant, getCurrentCaseName } from '@/lib/case-ledger'
...
function identifySource(source: string): string {
  ...
  const plaintiff = getCurrentPlaintiff().toLowerCase()
  const defendant = getCurrentDefendant().toLowerCase()

  // Plaintiff filings - check both entity name and common variations
  if (lower.includes(plaintiff.replace(' llc', '').replace(' ', '_')) ||
      lower.includes(plaintiff.replace(' ', '_')) ||
      lower.includes(plaintiff.replace(' ', ' '))) {
    return getCurrentPlaintiff()
  }

  // Defendant filings - check both entity name and common variations
  if (lower.includes(defendant.replace(' county', '').replace(' ', '_')) ||
      lower.includes(defendant.toLowerCase()) ||
      lower.includes("county's")) {
    return getCurrentDefendant()
  }
...
export async function searchAll(
  ...
  const commonNamespaces = ['legal-docs', getCurrentCaseName(), 'case-documents', 'documents']
